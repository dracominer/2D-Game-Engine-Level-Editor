...
public class MousePicker {
	...
	public void update(Camera cam, boolean changed) {
		if (changed) {
			viewMatrix = Maths.createViewMatrix(cam);
			currentRay = calculateMouseRay();
			if (intersectionInRange(0, RAY_RANGE, currentRay)) {
				currentWorldPoint = binarySearch(0, 0, RAY_RANGE, currentRay);
				//				System.out.println("Mouse world position is " + currentWorldPoint.x + ", " + currentWorldPoint.y);
			} else {
	...
	private Vector3f binarySearch(int count, float start, float finish, Vector3f ray) {
		...
		if (count >= RECURSION_COUNT) {
			...
			if (Math.abs(endPoint.z) <= CLOSE_ENOUGH) return endPoint;
			return null;
		}
